...
<task type="auto">
  ...
  <action>
    ...
    **2. skat_python.py — Compute ACAT-V in PurePythonSKATTest.run():**
    ...
    IMPORTANT: Also add `"acat_v_p": None` to ALL early-return `extra` dicts in
    PurePythonSKATTest.run() — these are at lines ~222-223 (empty genotype matrix)
    and ~241-243 (rank-deficient matrix). This ensures the `acat_v_p` key is always
    present in the result, avoiding inconsistent column presence in the output.
  ...
  <action>
    ...
    class TestComputeAcatV:
        ...
        def test_pure_python_skat_stores_acat_v_in_extra(self):
            ...
            # The null model is fit lazily on first run() call — no fit_null() method.
            contingency_data = {
