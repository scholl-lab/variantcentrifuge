...
<task type="auto">
  ...
  <action>
    ...
    3. **engine.py** — `from_names()` COAST auto block (lines 159-173):
       Replace the rpy2/AllelicSeries probe with:
       ```python
  ...
  <done>
    ...
    engine.from_names auto paths use Python implementations.
...
<task type="auto">
  ...
  <action>
    1. **test_json_config.py**: Update any tests that assert `skat_backend == "auto"` or
       `coast_backend == "auto"` as defaults to instead assert `"python"`. Specifically
       check line 186 (`assert config.skat_backend == "auto"`) and similar.
       Also update validation test at line 65 if it expects "auto" as default value.
  ...
  <done>
    test_json_config.py updated for new defaults.
