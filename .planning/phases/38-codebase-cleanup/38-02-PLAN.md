...
- `tests/test_cli.py` line 171: Update comment "Should fall back to original pipeline when config loading fails" — remove "original pipeline" terminology, e.g. "Should fall back to default behavior when config loading fails"
