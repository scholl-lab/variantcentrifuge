...
# NOTE on OMNI-01 (ACAT-V): OMNI-01 is satisfied by the cauchy_combination()
# infrastructure built in this plan. cauchy_combination() is the mathematical
# foundation shared by both ACAT-O (per-gene, this phase) and ACAT-V
# (per-variant, Phase 23). The actual per-variant ACAT-V computation is
# deferred to Phase 23 per CONTEXT.md scoping decisions.
...
<task type="auto">
  ...
  <action>
    ...
    2. Modify `run_all()`:
       - REMOVE the existing per-test FDR correction loop (lines ~204-221 in current code) — this is the ARCH-03 breaking change
    ...
    4. Inside `_compute_acat_o()`, add lazy import: `from variantcentrifuge.association.tests.acat import compute_acat_o`
       (Do NOT add a top-level import — keep it lazy inside the method to avoid import at module load.)
