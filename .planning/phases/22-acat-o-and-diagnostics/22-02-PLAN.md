  ...
  <action>
    ...
    **In `variantcentrifuge/stages/analysis_stages.py` (`AssociationAnalysisStage._process()`):**
    ...
    2. After `results_df = engine.run_all(gene_burden_data)` and the results check, add per-gene warning computation:
       - For each gene row in results_df, check case_carriers (from proband_carrier_count in gene_burden_data) against `assoc_config.min_case_carriers`
       - Build a "warnings" column: semicolon-separated warning flags per gene
