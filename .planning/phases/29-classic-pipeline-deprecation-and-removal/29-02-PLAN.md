...
<task type="auto">
  ...
  <action>
    ...
    2. Line 176: Update the docstring `"""Test that --use-new-pipeline flag takes precedence over config file."""` — this entire test may be testing phantom behavior. Read the test body to understand what it actually tests. If the test body references --use-new-pipeline as a CLI arg, remove that arg. If the test is entirely about verifying --use-new-pipeline behavior, consider removing the test or repurposing its docstring to describe what it actually verifies.
