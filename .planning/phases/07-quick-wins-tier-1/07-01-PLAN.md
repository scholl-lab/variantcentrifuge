...
<task type="auto">
  ...
  Step 2 — Verify the regression test actually covers the function under modification:
  Run `pytest --cov=variantcentrifuge.gene_burden --cov-report=term-missing tests/unit/test_gene_burden_regression.py` to confirm that `perform_gene_burden_analysis` is covered. The function must appear in the coverage output with meaningful line coverage. If not, fix the test until it does.

  Step 3 — Verify dead code then remove:
  In `variantcentrifuge/gene_burden.py`, the loop iterates gene_df rows parsing GT values into `case_samples_with_variants` and `control_samples_with_variants`, but these variables are NEVER used after the loop. The code says "Use the existing aggregated counts for now" and uses `proband_variant_count`, `proband_allele_count`, etc. from existing columns instead.

  Remove the entire dead code block: the variable declarations for `case_samples_with_variants`, `control_samples_with_variants`, `case_total_alleles`, `control_total_alleles` AND the entire `for _, row in gene_df.iterrows():` loop that populates them. Both the variable declarations and the loop are dead code because the variables are only used within the loop itself — nothing downstream references them.

  Keep the `for gene, gene_df in df.groupby("GENE"):` line and the lines immediately after it that read p_count and c_count. Keep the aggregation logic that uses existing columns (the "Use the existing aggregated counts" section and everything after it).

  Clean removal: no comments about what was removed, git history is the record.

  Step 4 — Verify regression test still passes after removal:
...
<task type="auto">
  ...
  <action>
  NOTE: These are two independent fixes grouped for efficiency. Either fix can be reverted without affecting the other.

  Fix 1 — gene_bed.py temp file leak:
  In `variantcentrifuge/gene_bed.py`, the `bed_path` created via `tempfile.mkstemp(suffix=".bed")` is never cleaned up. The `sorted_bed` and `merged_bed` are properly handled, but the original `bed_path` leaks.

  Read the file first to find the exact locations. Add `os.remove(bed_path)` immediately after the existing `os.remove(sorted_bed)` in the cleanup section, since `bed_path` is no longer needed once sorting is complete. The cleanup section should become:
  ```python
  ...
  Also wrap the entire temp file workflow in a try/finally block to ensure cleanup happens even on subprocess failures. The finally block should remove bed_path, sorted_bed, and merged_bed if they exist (using `os.path.exists()` checks before each `os.remove()`).

  Fix 2 — filters.py deprecated mktemp:
  In `variantcentrifuge/filters.py`, find the `tempfile.mktemp(suffix=".vcf")` call and replace:
  ```python
  ...
  This replaces the deprecated `mktemp` (security vulnerability — race condition between name generation and file creation) with the safe `mkstemp`. The existing cleanup (`os.remove(tmp_vcf)`) already handles deletion, so wrap the usage section in try/finally to ensure cleanup on subprocess failure:
  ```python
  tmp_fd, tmp_vcf = tempfile.mkstemp(suffix=".vcf")
  os.close(tmp_fd)
  try:
...
<success_criteria>
1. Dead GT parsing loop completely removed from gene_burden.py
