...
<task type="auto">
  ...
  The exact locations and transformations (from research):

  1. `variantcentrifuge/gene_burden.py:207` — `df.groupby("GENE")` → `df.groupby("GENE", observed=True)`
  2. `variantcentrifuge/stats_engine.py:157` — `df.groupby(groupby_cols)` → `df.groupby(groupby_cols, observed=True)`
  3. `variantcentrifuge/stats_engine.py:240` — `df.groupby(groupby_cols)` → `df.groupby(groupby_cols, observed=True)`
  4. `variantcentrifuge/stats.py:117` — add `observed=True`
  5. `variantcentrifuge/stats.py:151` — add `observed=True`
  6. `variantcentrifuge/stats.py:178` — add `observed=True`
  7. `variantcentrifuge/stages/analysis_stages.py:3068` — add `observed=True`
  8. `variantcentrifuge/inheritance/parallel_analyzer.py:167` — add `observed=True`
  9. `variantcentrifuge/inheritance/parallel_analyzer.py:221` — add `observed=True`
  10. `variantcentrifuge/inheritance/analyzer.py:101` — add `observed=True`
  11. `variantcentrifuge/inheritance/analyzer.py:104` — add `observed=True`
  12. `tests/performance/benchmark_comp_het.py:144` — add `observed=True`
  13. `tests/performance/benchmark_pipeline.py:376` — add `observed=True`
  14. `tests/performance/benchmark_pipeline.py:402` — add `observed=True`
  15. `tests/performance/benchmark_pipeline.py:433` — add `observed=True`
  16. `scripts/create_cohort_report.py:326` — add `observed=True`
  17. `scripts/create_cohort_report.py:329` — add `observed=True`

  IMPORTANT: Line numbers from research may have shifted slightly. Use `grep -n "\.groupby("` on each file to find exact current line numbers before editing.
...
<task type="auto">
  ...
  Modify `_execute_stage()` method in `variantcentrifuge/pipeline_core/runner.py` (currently at line 574). Add imports at top of file: `import gc` and `import psutil`.
  ...
  <done>gc.collect() runs after every pipeline stage. Memory before/after logged at DEBUG level via psutil. Pre-commit hook prevents new groupby calls without observed=True in variantcentrifuge/ and scripts/.</done>
</task>
...
5. Pre-commit hook catches future groupby calls missing observed=True
