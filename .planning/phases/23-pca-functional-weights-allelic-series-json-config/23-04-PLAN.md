...
@variantcentrifuge/config.py (load_config function)
</context>

<tasks>

<task type="auto">
  <name>Task 1: JSON config loading, validation, and CLI override in AssociationAnalysisStage</name>
  <files>
    variantcentrifuge/stages/analysis_stages.py
    tests/unit/test_json_config.py
  </files>
  <action>
    1. **Create `_build_assoc_config_from_context(context)`** helper function in analysis_stages.py (above AssociationAnalysisStage class or as a module-level function):
    ...
    **`_build_assoc_config_from_context(context)`**:
    ...
    2. **Replace inline AssociationConfig construction** in AssociationAnalysisStage._process():
    ...
    3. **Unit tests** in tests/unit/test_json_config.py:
  ...
  <done>
    ...
    Unit tests verify validation, precedence, and round-trip.
  </done>
</task>
