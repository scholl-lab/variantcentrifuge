...
<task type="auto">
  ...
  <action>
    ...
    3. **Wire PCA into AssociationAnalysisStage._process()** in analysis_stages.py:
       ...
       - Perform PCA load and merge INLINE — do NOT store the merged matrix in context.config. Instead, pass the result directly to the covariate_matrix variable that is used later in the genotype matrix building loop.
       ```python
       # PCA integration: load PCA file and merge with covariates inline
       ...
       if pca_file:
           ...
           # Merge with existing covariates (covariate_matrix is already loaded above)
           covariate_matrix, covariate_col_names = merge_pca_covariates(
               pca_matrix, pca_col_names, covariate_matrix,
               covariate_col_names if covariate_matrix is not None else []
           )
           logger.info(f"PCA: merged {len(pca_col_names)} PCs with existing covariates -> {len(covariate_col_names)} total columns")
       ```
       - This uses the local `covariate_matrix` variable that already exists in _process() and is later passed to gene_data["covariate_matrix"] in the genotype matrix loop (line ~2362). No need to persist in context.config.
  ...
  <done>
    ...
    AssociationAnalysisStage loads PCA file and merges with covariates inline (NOT stored in context.config).
    Merged covariate_matrix is used directly via the local variable that feeds gene_data["covariate_matrix"].
...
- PCA matrix NOT stored in PipelineContext — merged inline into local covariate_matrix variable
