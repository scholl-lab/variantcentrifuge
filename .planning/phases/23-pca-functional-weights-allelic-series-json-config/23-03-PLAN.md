...
must_haves:
  truths:
    ...
    - "COAST category weights are configurable via JSON config and --coast-weights CLI flag (default w=1,2,3)"
  ...
  key_links:
    ...
    - from: "variantcentrifuge/cli.py"
      to: "context.config['coast_weights']"
      via: "--coast-weights CLI arg parsed as comma-separated floats"
      pattern: "coast_weights"
---

<objective>
Implement the COAST allelic series test: variant classification (BMV/DMV/PTV) from annotation columns, R AllelicSeries::COAST() wrapper via rpy2, engine registry integration with ACAT-O feeding, and configurable category weights via JSON config and CLI flag.
...
@variantcentrifuge/cli.py (--association-tests help text, stats_group args)
@variantcentrifuge/association/tests/__init__.py (verify exists for allelic_series import)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create COASTTest with variant classification, rpy2 wrapper, and --coast-weights CLI arg</name>
  <files>
    variantcentrifuge/association/tests/allelic_series.py
    variantcentrifuge/association/tests/__init__.py
    variantcentrifuge/association/base.py
  ...
  <action>
    ...
    **COASTTest** class (AssociationTest subclass):
    ...
    2. **Verify `variantcentrifuge/association/tests/__init__.py` exists** — if missing, create it as empty file. If it exists, ensure `allelic_series` is importable from the package (no explicit import needed if __init__.py exists, but verify).

    3. **Extend AssociationConfig** in base.py:
    ```python
    coast_weights: list[float] | None = None
    """Category weights for COAST allelic series (default: [1.0, 2.0, 3.0] for BMV, DMV, PTV)."""
    ```

    4. **Add --coast-weights CLI arg** in cli.py stats_group (after --variant-weight-params):
    ```python
    ...
    ```
    Parse in validation section:
    ```python
    if getattr(args, "coast_weights", None):
        try:
            args.coast_weights = [float(x) for x in args.coast_weights.split(",")]
            if len(args.coast_weights) != 3:
                parser.error("--coast-weights must have exactly 3 values (BMV, DMV, PTV)")
        except ValueError:
            parser.error("--coast-weights values must be numeric")
    ```
    Wire to context.config:
    ```python
    "coast_weights": getattr(args, "coast_weights", None),
    ```

    5. **Update CLI** --association-tests help to include "coast":
    Update the help string to list coast as available test.

    6. **Add `coast` pytest marker** to pyproject.toml in the `[tool.pytest.ini_options]` markers list:
    ```
  ...
  <done>
    ...
    CLI has --coast-weights arg (comma-separated floats) and updated --association-tests help.
    coast pytest marker registered in pyproject.toml.
    variantcentrifuge/association/tests/__init__.py verified to exist.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    - Mark tests with `@pytest.mark.unit` and `@pytest.mark.coast`.
...
- Category weights configurable via --coast-weights CLI flag AND JSON config (default 1,2,3)
