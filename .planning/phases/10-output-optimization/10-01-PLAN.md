...
must_haves:
  truths:
    ...
    - "append_tsv_as_sheet remains openpyxl append mode (xlsxwriter cannot open existing files)"
  ...
  key_links:
    ...
    - from: "variantcentrifuge/converter.py:append_tsv_as_sheet"
      to: "openpyxl append mode"
      via: "opens existing xlsx with engine='openpyxl', mode='a'"
      pattern: "mode.*a.*openpyxl"
---
...
<task type="auto">
  <name>Task 1: Add xlsxwriter dependency and refactor convert_to_excel</name>
  <files>
    pyproject.toml
    variantcentrifuge/converter.py
  </files>
...
3. No changes to `append_tsv_as_sheet` -- xlsxwriter cannot open existing files, so openpyxl append mode is required and correct. The main speedup comes from the Results sheet (largest sheet by far). Metadata/Statistics/Gene Burden sheets are tiny.
