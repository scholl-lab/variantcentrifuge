...
18b. `test_test_gene_cleans_r_globals_per_gene`:
    - Mock ro.r to capture calls
    - Call test_gene() once
    - Assert `ro.r("rm(list=ls(pattern='\\\\._vc_'))")` was called (R globals cleaned up)
    - Assert `del r_z` equivalent — the R matrix reference is not held after return
...
6. `test_run_returns_skat_extra_columns`:
   - Mock backend.test_gene to return {"p_value": 0.03, "rho": 0.5, "warnings": ["w1"], "n_marker_test": 5}
   - Assert result.extra contains skat_o_rho, skat_warnings, skat_method, skat_n_marker_test
