...
must_haves:
  truths:
    ...
    - "R warnings captured per gene via withCallingHandlers (verified by mocked tests in Plan 20-03)"
    - "Statistical NA (from R NA_Real) mapped to p_value=None (verified by mocked tests in Plan 20-03)"
  artifacts:
...
could conditionally return True.
  ...
  <verify>
    ...
    - `python -c "import inspect; from variantcentrifuge.association.engine import AssociationEngine; src = inspect.getsource(AssociationEngine.run_all); assert 'prepare' in src and 'finalize' in src; print('OK')"` confirms engine calls prepare/finalize hooks
    - `python -c "from variantcentrifuge.association.base import AssociationTest; assert hasattr(AssociationTest, 'prepare') and hasattr(AssociationTest, 'finalize'); print('OK')"` confirms ABC has default no-ops
    - `make lint` passes
