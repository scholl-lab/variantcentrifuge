...
def analyze_inheritance_parallel(
    ...
    logger.info(
        f"Starting parallel inheritance analysis for {len(df)} variants across {len(sample_list)} samples"
    )
    ...
    ) and len(df) >= min_variants_for_parallel and "GENE" in df.columns
    
    if use_parallel and "GENE" in df.columns:
        # Group by gene
        gene_groups = df.groupby("GENE")
        genes_with_multiple_variants = [
            (gene, group_df) for gene, group_df in gene_groups 
