...
def analyze_gene_for_compound_het_vectorized(
    ...
    for sample_id in sample_list:
        ...
        has_parents = (
            father_id and mother_id and 
            father_id in sample_list and 
            mother_id in sample_list
        )
        ...
        if has_parents:
            ...
            if is_affected(sample_id, pedigree_data):
                comp_het_pairs = [(i, j) for i in range(len(het_indices)) 
        ...
        for var1_idx, var2_idx in comp_het_pairs:
            ...
            if has_parents:
                trans_config, comp_het_type = determine_compound_het_type_vectorized(
                    var1_idx, var2_idx, 
                    father_genotypes, mother_genotypes,
                    gene_df, sample_id, pedigree_data
                )
            else:
...
def find_trans_pairs_vectorized(
    ...
    for i in range(n_het):
        for j in range(i + 1, n_het):
            ...
            pattern1 = (
                father_has_var[i] and not mother_has_var[i] and
                not father_has_var[j] and mother_has_var[j]
            )
            pattern2 = (
                not father_has_var[i] and mother_has_var[i] and
                father_has_var[j] and not mother_has_var[j]
            )
...
def create_variant_key_fast(df: pd.DataFrame, idx: int) -> str:
    ...
    """
    row = df.iloc[idx]
    return f"{row.get('CHROM', 'chr?')}:{row.get('POS', '0')}:{row.get('REF', 'N')}>{row.get('ALT', 'N')}"
...
def determine_compound_het_type_vectorized(
    ...
    # Clear trans pattern
    if (var1_from_father and not var1_from_mother and 
        not var2_from_father and var2_from_mother):
        return ("trans", "compound_heterozygous")
    
    if (not var1_from_father and var1_from_mother and 
        var2_from_father and not var2_from_mother):
        return ("trans", "compound_heterozygous")
    
    # Clear cis pattern (both from same parent)
    if ((var1_from_father and var2_from_father and 
         not var1_from_mother and not var2_from_mother) or
        (var1_from_mother and var2_from_mother and 
         not var1_from_father and not var2_from_father)):
        return ("cis", "not_compound_heterozygous")
    
    # Ambiguous cases
    if (var1_from_father and var1_from_mother and 
        var2_from_father and var2_from_mother):
        return ("ambiguous", "compound_heterozygous_possible")
    
    if ((var1_from_father and var1_from_mother) or 
        (var2_from_father and var2_from_mother)):
        return ("ambiguous", "compound_heterozygous_possible")
...
def analyze_gene_for_compound_het(
    ...
    """
    Wrapper function that can use either vectorized or original implementation.
    ...
    """
    if use_vectorized:
        return analyze_gene_for_compound_het_vectorized(
            gene_df, pedigree_data, sample_list
        )
    else:
        # Fall back to original implementation
        from . import comp_het
        return comp_het.analyze_gene_for_compound_het(
            gene_df, pedigree_data, sample_list
        )