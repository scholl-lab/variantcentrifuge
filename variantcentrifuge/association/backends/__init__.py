...
def get_skat_backend(backend_name: str) -> SKATBackend:
    ...
    backend_name : str
        ...
        - ``"auto"``   — Use Python backend (default). R backend available via
                         ``"r"`` (deprecated, will be removed in v0.17.0).
    ...
    if backend_name == "auto":
        # "auto" now maps directly to Python backend (default since v0.15.0).
        # R backend remains available via explicit --skat-backend r.
        from variantcentrifuge.association.backends.python_backend import PythonSKATBackend
