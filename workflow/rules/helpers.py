...
def build_vc_flags(vc_config: dict[str, Any], igv_config: dict[str, Any]) -> list[str]:
    ...
    if profile:
        flags.append(f"--field-profile {profile}")
    ...
    if gt_fields:
        flags.append("--append-extra-sample-fields " + " ".join(gt_fields))

    for gl in vc_config.get("gene_list_files", []):
        flags.append(f"--annotate-gene-list {gl}")
    ...
    if igv_config.get("enabled"):
        ...
        if ref:
            flags.append(f"--igv-reference {ref}")
        bam = igv_config.get("bam_mapping_file")
        if bam:
            flags.append(f"--bam-mapping-file {bam}")
        fasta = igv_config.get("fasta")
        if fasta:
            flags.append(f"--igv-fasta {fasta}")
        ideogram = igv_config.get("ideogram")
        if ideogram:
            flags.append(f"--igv-ideogram {ideogram}")
        flags.append(f"--igv-flanking {igv_config.get('flanking', 50)}")
