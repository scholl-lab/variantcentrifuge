...
def verify_reference_integrity(fasta_file, fai_file):
    ...
    if len(lines) < 20:  # Should have 25+ chromosomes/contigs including decoys
        logger.warning(f"Index file has only {len(lines)} sequences, expected 25+")
    
    logger.info(f"Integrity checks passed:")
    logger.info(f"  - FASTA: {fasta_size_gb:.2f} GB")
    logger.info(f"  - Index: {len(lines)} sequences")
...
def main():
    ...
    parser.add_argument(
        "--compressed-only", action="store_true",
    ...
    if not args.compressed_only:
        ...
        if not verify_reference_integrity(fasta_file, fai_file):
