    ...
    for tool in required_tools:
        try:
            ...
            if tool == "gatk":
                ...
                logger.info(f"Found GATK: {version_output.split()[0] if version_output else 'Unknown version'}")
            else:
...
def find_bam_files(data_dir):
    ...
    bam_files = []
    for bam_path in data_dir.glob("*.HG19.exome.bam"):
...
def find_reference_files(reference_dir):
    ...
    if not dict_file.exists():
        ...
        logger.info("Tip: Use 'python download_reference_genome.py' to download the pre-built dictionary")
...
def convert_bed_to_interval_list(bed_file, reference_fasta, output_dir):
    ...
    cmd = [
        ...
        "-I", str(sorted_bed),
        "-O", str(interval_list),
        "-SD", str(reference_fasta.with_suffix(".dict"))
    ]
...
def call_variants_multisample(bam_files, reference_fasta, interval_list, output_dir, java_memory="4g"):
    ...
    cmd = [
        "gatk", f"--java-options", f"-Xmx{java_memory}",
        "HaplotypeCaller",
    ...
    logger.info(f"Command: gatk --java-options -Xmx{java_memory} HaplotypeCaller -R [ref] -L [intervals] -O {output_vcf.name} [multiple -I inputs]")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Call variants on GIAB BAM files using GATK HaplotypeCaller")
    parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="Directory containing BAM files")
    parser.add_argument("--reference-dir", type=Path, default=DEFAULT_REFERENCE_DIR, help="Directory containing reference genome")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR, help="Output directory for VCF files")
    parser.add_argument("--bed-file", type=Path, default=DEFAULT_BED_FILE, help="BED file with regions to call")
    ...
    logger.info(f"Parameters: data_dir={args.data_dir}, reference_dir={args.reference_dir}, output_dir={args.output_dir}")
    ...
    output_vcf = call_variants_multisample(bam_files, reference_fasta, interval_list, args.output_dir, args.java_memory)
