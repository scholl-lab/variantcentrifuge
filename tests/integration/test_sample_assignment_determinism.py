...
from variantcentrifuge.pipeline_refactored import run_pipeline
...
class TestPipelineSampleDeterminism:
    ...
    def test_multiple_pipeline_runs_identical_results(self, mock_exists, mock_extract, 
                                                     mock_get_names, temp_workspace, mock_test_data):
        ...
        for run_num in range(3):
            ...
            workspace = Workspace(str(run_workspace))
            context = PipelineContext(config, workspace)
            
            with patch('variantcentrifuge.stages.setup_stages.Path.exists', return_value=True):
                ...
                from variantcentrifuge.stages.processing_stages import FieldExtractionStage, GenotypeReplacementStage
                ...
                result_data = {
                    ...
                    "genotype_file_exists": context.genotype_replaced_tsv and context.genotype_replaced_tsv.exists()
                }
        ...
        for i, result in enumerate(results[1:], 1):
            assert result["vcf_samples"] == first_result["vcf_samples"], \
                f"Run {i+1} sample order differs from run 1"
            
            if first_result["genotype_file_exists"] and result["genotype_file_exists"]:
                assert result["genotype_content"] == first_result["genotype_content"], \
    ...
    def test_sample_order_propagation_through_stages(self, temp_workspace, mock_test_data):
        ...
        with patch('variantcentrifuge.helpers.get_vcf_names', return_value=test_samples):
            ...
            context = PipelineContext(mock_test_data["config"], workspace)
    ...
    def test_genotype_replacement_determinism(self, mock_get_names, temp_workspace):
        ...
        for i, result in enumerate(results[1:], 1):
            assert result == first_result, \
                f"Genotype replacement run {i+1} differs from run 1"
    ...
    def test_inheritance_analysis_determinism_with_real_data(self, temp_workspace):
        ...
        pedigree_data = {
            "Child": {"sample_id": "Child", "father": "Father", "mother": "Mother", "affected": True},
        ...
        for _ in range(3):
            ...
            if result_context.current_dataframe is not None:
                ...
                inheritance_cols = [col for col in result_context.current_dataframe.columns 
                                  if 'Inheritance' in col]
                if inheritance_cols:
                    inheritance_result = result_context.current_dataframe[inheritance_cols].to_dict('records')
