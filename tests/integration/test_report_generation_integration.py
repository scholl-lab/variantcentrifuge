...
"""
    ...
    def test_igv_report_stage_integration(
        self, temp_output_dir, sample_dataframe, simple_bam_mapping
    ):
    ...
    def test_html_report_stage_integration(self, temp_output_dir, sample_dataframe):
        ...
        # Create test context
        context = create_test_context(
            output_dir=str(temp_output_dir), config_overrides={"html_report": True}
        )
        context.current_dataframe = sample_dataframe
        context.mark_complete("tsv_output")
    ...
    def test_combined_reports_integration(
        self, temp_output_dir, sample_dataframe, simple_bam_mapping
    ):
    ...
    def test_missing_dependencies_handling(self, temp_output_dir):
        ...
        stage = IGVReportStage()
        with pytest.raises(
            RuntimeError, match="Stage 'igv_report' requires these stages to complete first"
        ):
            stage(context)
