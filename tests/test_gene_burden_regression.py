...
def test_proband_count_not_zero_regression():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd = [
            ...
            str(test_files["vcf"]),
            "--gene-file",
            str(test_files["genes"]),
            "--case-samples-file",
            str(test_files["case_samples"]),
            "--control-samples-file",
            str(test_files["control_samples"]),
            "--perform-gene-burden",
            "--preset",
            "high_or_moderate",
            "--output-dir",
        ...
        assert (
            proband_counts[0] > 0
        ), f"REGRESSION: proband_count is {proband_counts[0]}, should be > 0"
        ...
        assert (
            control_counts[0] > 0
        ), f"REGRESSION: control_count is {control_counts[0]}, should be > 0"
        ...
        assert (
            proband_counts[0] == expected_case_count
        ), f"Expected {expected_case_count} cases, got {proband_counts[0]}"
        assert (
            control_counts[0] == expected_control_count
...
def test_sample_assignment_with_all_vcf_samples():
    ...
    correct_result = assign_case_control_counts(
        df, case_samples, control_samples, correct_all_samples
    )
    ...
    assert (
        buggy_result["proband_variant_count"].iloc[0]
        == correct_result["proband_variant_count"].iloc[0]
    )
    assert (
        buggy_result["control_variant_count"].iloc[0]
        == correct_result["control_variant_count"].iloc[0]
    )
