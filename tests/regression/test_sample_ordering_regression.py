...
""",
    ...
    def test_old_vs_new_pipeline_sample_order_consistency(self, sample_test_data):
        ...
        with patch('variantcentrifuge.helpers.get_vcf_names', return_value=sample_test_data["vcf_samples"]):
            new_pipeline_samples = get_vcf_samples("dummy.vcf")
        
        # New pipeline should return the expected sample order
        assert new_pipeline_samples == sample_test_data["vcf_samples"], \
    ...
    def test_sample_list_propagation_determinism(self):
        ...
        assert all(config == first_config for config in config_sample_lists), \
        ...
        assert all(parsed == first_parsed for parsed in parsed_lists), \
    ...
    def test_no_random_sample_assignment_regression(self):
        ...
        with patch('variantcentrifuge.helpers.get_vcf_names', return_value=test_samples):
            ...
            assert first_result == test_samples, \
    ...
    def test_set_conversion_safety_net(self):
        ...
        context = PipelineContext({"inheritance_mode": "simple"}, workspace)
