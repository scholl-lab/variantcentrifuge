    ...
    def compare_json_files(
        ...
        try:
            ...
            self._compare_json_objects(
                old_data, new_data, "", differences, ignore_keys
            )
    ...
    def _compare_json_objects(
        ...
        if isinstance(old_obj, dict):
            ...
            if len(old_obj) != len(new_obj):
                differences.append(
                    f"{path}: List length differs - old={len(old_obj)}, "
                    f"new={len(new_obj)}"
                )
            else:
            ...
            if abs(old_obj - new_obj) > 1e-6:
                differences.append(
                    f"{path}: Value differs - old={old_obj}, new={new_obj}"
                )
