...
def test_data_dir():
    ...
    if not fixtures_dir.exists():
        pytest.skip(f"Test data not found at {fixtures_dir}. Run generate_comprehensive_test_data.py first.")
    return fixtures_dir
...
class TestGeneBurdenDirectSamples:
    ...
    def test_direct_sample_specification(self, vcf_file, gene_file, test_data_dir):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            control_samples = (test_data_dir / "control_samples.txt").read_text().strip().split('\n')[:8]
            
            cmd = [
                "variantcentrifuge",
                "--vcf-file", str(vcf_file),
                "--gene-file", str(gene_file),
                "--case-samples", ",".join(case_samples),
    ...
    def test_sample_file_specification(self, vcf_file, gene_file, test_data_dir):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                ...
                "--vcf-file", str(vcf_file),
                "--gene-file", str(gene_file),
                "--case-samples-file", str(case_file),
                "--control-samples-file", str(control_file),
                "--perform-gene-burden",
                "--preset", "rare,coding",
                "--output-dir", tmpdir,
                "--output-file", "results.tsv",
...
class TestGeneBurdenPhenotypeBased:
    ...
    def test_hpo_based_classification(self, vcf_file, gene_file, test_data_dir):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            control_hpo_terms = (test_data_dir / "control_hpo_terms.txt").read_text().strip().split('\n')
            ...
            cmd = [
                ...
                "--case-phenotypes", ",".join(case_hpo_terms),
                "--control-phenotypes", ",".join(control_hpo_terms),
                "--perform-gene-burden",
                "--preset", "rare,coding",
                "--output-dir", tmpdir,
                "--output-file", "results.tsv",
    ...
    def test_hpo_term_files(self, vcf_file, gene_file, test_data_dir):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                ...
                "--phenotype-sample-column", "SampleID",
                "--phenotype-value-column", "identifier",
                "--case-phenotypes-file", str(case_hpo_file),
                "--control-phenotypes-file", str(control_hpo_file),
                "--perform-gene-burden",
                "--preset", "rare,coding",
                "--output-dir", tmpdir,
                "--output-file", "results.tsv",
    ...
    def test_alternative_column_names(self, vcf_file, gene_file, test_data_dir):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                ...
                "--gene-file", str(gene_file),
                "--phenotype-file", str(phenotype_file),
                "--phenotype-sample-column", "sample_name",  # Alternative column name
                "--phenotype-value-column", "hpo_id",        # Alternative column name
                "--case-phenotypes", ",".join(case_hpo_terms),
                "--control-phenotypes", ",".join(control_hpo_terms),
                "--perform-gene-burden",
                "--preset", "rare,coding",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if results_file.exists():
                ...
                if gene_col:
                    ...
                    assert len(found_disease_genes) > 0, f"No disease genes found in results: {result_genes}"
    ...
    def test_missing_phenotype_file(self, vcf_file, gene_file):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            cmd = [
                ...
                "--vcf-file", str(vcf_file),
                "--gene-file", str(gene_file),
                "--phenotype-file", "nonexistent_file.csv",
                "--phenotype-sample-column", "SampleID",
                "--phenotype-value-column", "identifier",
                "--case-phenotypes", "HP:0000113",
                "--control-phenotypes", "HP:0000001",
                "--perform-gene-burden",
                "--preset", "rare,coding",
                "--output-dir", tmpdir,
                "--output-file", "results.tsv",
...
def test_test_data_availability():
    ...
    if not fixtures_dir.exists():
        pytest.skip(f"Test data not generated. Run: cd {fixtures_dir.parent} && python generate_comprehensive_test_data.py --output-dir comprehensive_test_data")
