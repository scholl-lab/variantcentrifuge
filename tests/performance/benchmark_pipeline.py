...
class PerformanceBenchmark:
    ...
    def generate_report(self, df: pd.DataFrame) -> Dict:
        ...
        for test_name in summary_df["test_name"].unique():
            old_stats = summary_df[
                ...
                (summary_df["pipeline"] == "old")
            ]
            new_stats = summary_df[
                (summary_df["test_name"] == test_name) &
                (summary_df["pipeline"] == "new")
            ]
            
            if len(old_stats) > 0 and len(new_stats) > 0:
                speedup = old_stats.iloc[0]["avg_time"] / new_stats.iloc[0]["avg_time"]
                memory_ratio = new_stats.iloc[0]["avg_memory_mb"] / old_stats.iloc[0]["avg_memory_mb"]
    ...
    def plot_results(self, df: pd.DataFrame):
        ...
        summary = df.groupby(["test_name", "pipeline_type"])["execution_time"].agg(["mean", "std"]).reset_index()
        ...
        memory_summary = df.groupby(["test_name", "pipeline_type"])["peak_memory_mb"].mean().reset_index()
        
        old_memory = memory_summary[memory_summary["pipeline_type"] == "old"]["peak_memory_mb"].values
        new_memory = memory_summary[memory_summary["pipeline_type"] == "new"]["peak_memory_mb"].values
        ...
        if len(thread_tests) > 0:
            ...
            for pipeline in ["old", "new"]:
                ...
                ax.plot(thread_summary.index, thread_summary.values, 
        ...
        speedup_data = []
        for test_name in df["test_name"].unique():
            old_time = df[(df["test_name"] == test_name) & (df["pipeline_type"] == "old")]["execution_time"].mean()
            new_time = df[(df["test_name"] == test_name) & (df["pipeline_type"] == "new")]["execution_time"].mean()
...
def main():
    ...
    cancer_genes = [
        "ABL1", "AKT1", "ALK", "APC", "AR", "ATM", "BRAF", "BRCA1", "BRCA2",
        "CDH1", "CDKN2A", "CTNNB1", "EGFR", "ERBB2", "EZH2", "FLT3", "GATA3",
        "HER2", "IDH1", "JAK2", "KIT", "KRAS", "MET", "MLH1", "MYC", "NF1",
        "NOTCH1", "NPM1", "NRAS", "PALB2", "PDGFRA", "PIK3CA", "PTEN", "PTPN11",
        "RB1", "RET", "RUNX1", "SMAD4", "STK11", "TP53", "TSC1", "VHL", "WT1",
    ]
    ...
    for test_result in report['summary']['speedup_by_test']:
        print(f"  {test_result['test_name']}: {test_result['speedup']:.2f}x "
