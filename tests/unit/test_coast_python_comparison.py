...
#
# Scenario inputs use the SAME genotype matrices as R (exported as CSV fixtures
# by scripts/generate_coast_golden.R). This ensures Python and R compute on
# identical data, making p-value comparison meaningful.
#
# r_omnibus_p values are from R AllelicSeries v0.1.1.5
# (generated 2026-02-24 via: Rscript scripts/generate_coast_golden.R).
# ---------------------------------------------------------------------------
...
_GCKD_FIXTURE_DIR = pathlib.Path(__file__).parent.parent / "fixtures" / "coast_golden"


def _load_gckd_fixture(
    scenario_name: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Load R-exported genotype/phenotype/annotation matrices for a GCKD scenario.

    The CSV files are generated by scripts/generate_coast_golden.R to ensure
    Python and R COAST implementations are compared on identical input data.
    """
    geno_path = _GCKD_FIXTURE_DIR / f"{scenario_name}_geno.csv"
    pheno_path = _GCKD_FIXTURE_DIR / f"{scenario_name}_pheno.csv"
    anno_path = _GCKD_FIXTURE_DIR / f"{scenario_name}_anno.csv"
    ...
    pheno = pd.read_csv(pheno_path)["pheno"].values.astype(np.float64)
    anno = pd.read_csv(anno_path)["anno"].values.astype(np.int32)

    return geno, anno, pheno
...
# r_omnibus_p: from R AllelicSeries v0.1.1.5 (Rscript scripts/generate_coast_golden.R).
# Golden values from R AllelicSeries v0.1.1.5 and Python COAST on identical
# fixture data (tests/fixtures/coast_golden/).  python_omnibus_p is the
# regression target; r_omnibus_p documents the R reference.
_GCKD_GOLDEN_SCENARIOS: dict[str, dict] = {
    "gckd_g1_all_categories": {
        "python_omnibus_p": 1.007051540477277e-01,
        "r_omnibus_p": 1.022380203777098e-01,
        "description": "All 3 categories (7 BMV + 2 DMV + 1 PTV), PKD1-like gene.",
    },
    "gckd_g2_partial_bmv_ptv": {
        "python_omnibus_p": 8.440943531121691e-01,
        "r_omnibus_p": 7.047427129147510e-01,
        "description": "Partial categories (5 BMV + 0 DMV + 2 PTV), missing DMV.",
    },
    "gckd_g3_single_variant": {
        "python_omnibus_p": 7.957670583340543e-02,
        "r_omnibus_p": 3.672717286639919e-02,
        "description": "Single variant (PTV only), rank-1 genotype matrix edge case.",
    },
    "gckd_g4_all_bmv": {
        "python_omnibus_p": 6.742124026912270e-01,
        "r_omnibus_p": 5.003489094113375e-01,
        "description": "All 8 variants BMV only (COL4A5-like), no PTV.",
    },
    "gckd_g5_tied_scores": {
        "python_omnibus_p": 5.073739185266731e-01,
        "r_omnibus_p": 4.202313350300252e-01,
        "description": "Equal MAF across all categories, tied SIFT/PolyPhen scores.",
    },
}
...
class TestCOASTGCKDGoldenComparison:
    """Regression tests for Python COAST against golden values (TD-03).

    Both R and Python run on identical input data exported as CSV fixtures
    in tests/fixtures/coast_golden/ (generated by scripts/generate_coast_golden.R).

    Python is tested against its own golden values (tight 1e-6 tolerance) for
    regression detection. R golden values are stored for reference but not
    strictly compared — the implementations use different SKAT kernels and
    ACAT combination methods, producing order-of-magnitude agreement but
    not <10% match on edge cases (partial categories, single variants).

    To regenerate fixtures: Rscript scripts/generate_coast_golden.R
    """
    ...
    def test_gckd_scenario_python_p_valid(self, scenario_name: str) -> None:
        """Python COAST returns a valid omnibus p-value in (0, 1]."""
        geno, anno, pheno = _load_gckd_fixture(scenario_name)
    ...
    @pytest.mark.parametrize("scenario_name", list(_GCKD_GOLDEN_SCENARIOS.keys()))
    def test_gckd_scenario_regression(self, scenario_name: str) -> None:
        """Python omnibus p matches its own golden value (regression guard)."""
        scenario = _GCKD_GOLDEN_SCENARIOS[scenario_name]
        expected_p = scenario["python_omnibus_p"]
        ...
        if python_p is None:
            pytest.skip(f"[{scenario_name}] COAST returned None (gene skipped).")

        rel_err = abs(python_p - expected_p) / expected_p
        assert rel_err < 1e-6, (
            f"[{scenario_name}] Python regression failure: "
            f"got={python_p:.6e}, expected={expected_p:.6e}, rel_err={rel_err:.2e}"
        )

    @pytest.mark.parametrize("scenario_name", list(_GCKD_GOLDEN_SCENARIOS.keys()))
    def test_gckd_scenario_deterministic(self, scenario_name: str) -> None:
        """Two calls with same input yield same p-value."""
        geno, anno, pheno = _load_gckd_fixture(scenario_name)
