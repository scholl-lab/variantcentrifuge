...
class TestCLIDebugLogging:
    ...
    def test_command_line_logging_in_debug_mode(self, mock_validate_params, mock_validate_vcf, mock_run_pipeline, caplog):
    ...
    @patch('variantcentrifuge.cli.validate_vcf_file')
    @patch('variantcentrifuge.cli.validate_mandatory_parameters')
    def test_no_command_line_logging_in_info_mode(self, mock_validate_params, mock_validate_vcf, mock_run_pipeline, caplog):
        ...
        test_args = [
            'variantcentrifuge',
            '--log-level', 'INFO',  # INFO level, not DEBUG
            '-v', '/tmp/test.vcf',
            '-r', 'GRCh37'
        ]
