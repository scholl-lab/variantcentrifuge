...
def test_gene_burden_sample_assignment(test_data_paths):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd = [
            "variantcentrifuge",
            "--vcf-file", str(test_data_paths["vcf"]),
            "--gene-file", str(test_data_paths["genes"]),
            "--case-samples-file", str(test_data_paths["case_samples"]),
            "--control-samples-file", str(test_data_paths["control_samples"]),
            "--perform-gene-burden",
            "--preset", "high_or_moderate",
            "--output-dir", str(output_dir),
            "--output-file", "test_results.tsv",
            "--use-new-pipeline",
            "--log-level", "INFO"
        ]
        ...
        required_columns = [
            "GENE", "proband_count", "control_count",
        ...
        assert df["control_count"].iloc[0] > 0, "Control count is zero - control samples not assigned"
...
def test_gene_burden_determinism(test_data_paths):
    ...
    for run_num in range(3):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                ...
                "--preset", "high_or_moderate",
                "--output-dir", str(output_dir),
                "--output-file", f"run_{run_num}_results.tsv",
                "--use-new-pipeline",
    ...
    for i, df in enumerate(results[1:], 1):
        ...
        pd.testing.assert_series_equal(base_df["GENE"], df["GENE"], 
                                     check_names=False, 
                                     obj=f"Gene order differs between run 1 and run {i+1}")
        
        # Check identical statistical results
        for col in ["proband_count", "control_count", "proband_allele_count", 
                   "control_allele_count", "raw_p_value", "corrected_p_value"]:
            pd.testing.assert_series_equal(base_df[col], df[col],
                                         check_names=False,
                                         obj=f"Column {col} differs between run 1 and run {i+1}")
        
        # Check odds ratios (may have floating point differences)
        pd.testing.assert_series_equal(base_df["odds_ratio"], df["odds_ratio"],
                                     check_names=False, 
                                     check_exact=False,
...
def test_assign_case_control_counts_unit():
    ...
    expected_columns = [
        ...
        "proband_allele_count", "control_allele_count",
        "proband_homozygous_count", "control_homozygous_count"
    ]
